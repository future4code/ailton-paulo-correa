...
export class PostBusiness {
  ...
  public getPosts = async (input: IGetPostsInputDTO) => {
    ...
    const posts = postsDB.map(
      (postDB) => new Post(postDB.id, postDB.content, postDB.user_id)
    );
  ...
  public addLike = async (input: IAddLikeInputDTO) => {
    ...
    if (isAlreadyLiked) throw new ConflictError("Já deu like");
  ...
  public removeLike = async (input: IRemoveLikeInputDTO) => {
    ...
    if (!isAlreadyLiked) throw new NotFoundError("Ainda não deu like");
